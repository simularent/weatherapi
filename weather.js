...
let url = `http://api.openweathermap.org/data/2.5/weather?q=${usercity}&units=imperial&appid=${apiKey}`;
...
app.get('/temperature', (req, res) => {
	usercity = req.query.city || 'Portland';
...
function testdate () {
	...
	db.each('SELECT temperature, city, query_time, id FROM weather ORDER BY id DESC LIMIT 1', function(err, row) {
 	var date = new Date();
	//data from database
	...
	if((date - new Date(row.query_time)) > FIVE_MIN) {
	   runrequest();
		}
	});
};
